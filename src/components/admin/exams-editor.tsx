...
export function ExamsEditor({
  ...
  const filteredExams = exams.filter(
    (exam) => !filterStatus || exam.status === filterStatus,
  );
  ...
  const bulk = useBulkSelection(filteredExams);
              ...
              {filteredExams.map((exam) => (
                <tr key={exam.id}>
                  <td className="admin-cell-nowrap">
                    <input
                      type="checkbox"
                      checked={bulk.selected.has(exam.id)}
                      onChange={() => bulk.toggle(exam.id)}
                      aria-label={messages.common.selectAllLabel}
                    />
                  </td>
                  <td>{exam.code}</td>
