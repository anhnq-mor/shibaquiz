...
import { saveTopicSchema } from "@/domain/admin/content";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const input = await parseJson(request, saveTopicSchema);
    const id = await getAdminContentService().saveTopic(input, admin.id);
