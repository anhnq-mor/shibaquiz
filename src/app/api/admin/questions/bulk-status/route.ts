...
import { bulkStatusSchema } from "@/domain/admin/content";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const input = await parseJson(request, bulkStatusSchema);
    const results = await getAdminContentService().bulkSetQuestionStatus(
      input.ids,
      input.status,
      admin.id,
    );
