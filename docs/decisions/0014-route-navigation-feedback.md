...
Navigation now responds promptly on slow or uncached routes without flashing a loader for fast transitions. The shared wrapper must be used for new internal route links, while downloads, external URLs, and hash anchors continue to use native anchors.
