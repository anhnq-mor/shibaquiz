...
| S10-10 | Replace generic loaders with the animated Shiba experience         | Route/API loading uses bilingual mission copy and a code-native running Shiba; API completion briefly stops and wags; reduced-motion users get a static state                                              | DONE                                                                                                                 |
...
| S11-10 | Preserve the selected import file through Review             | Confirm uses client state rather than an unmounted input ref and reports missing state instead of silently returning | DONE   |
