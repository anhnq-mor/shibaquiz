import type { EmailService, Locale } from "../domain/auth";
import type { AuthConfig } from "../env";

function copy(productName: string, locale: Locale) {
  return locale === "vi"
    ? {
        verifySubject: `Xác minh tài khoản ${productName}`,
        verifyHeading: "Xác minh địa chỉ email",
        verifyBody: "Liên kết này có hiệu lực trong 24 giờ.",
        resetSubject: `Đặt lại mật khẩu ${productName}`,
    ...
    : {
        verifySubject: `Verify your ${productName} account`,
        verifyHeading: "Verify your email address",
        verifyBody: "This link is valid for 24 hours.",
        resetSubject: `Reset your ${productName} password`,
...
export class AuthEmailService implements EmailService {
  constructor(
    ...
    private readonly productName = "the app",
  ) {}

  sendVerification(
    input: Parameters<EmailService["sendVerification"]>[0],
  ): Promise<void> {
    const messages = copy(this.productName, input.locale);
  ...
  private async send(input: {
    ...
    if (this.config.EMAIL_PROVIDER === "console") {
      console.info(
        `[${this.productName} email] ${input.subject} -> ${input.to}\n${input.url}`,
      );
      return;
    }
