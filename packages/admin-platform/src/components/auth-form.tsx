...
import type { Locale, UserRole } from "../domain/auth";
...
export interface AuthFormMessages {
  ...
  login: { action: string };
  forgot: { action: string; success: string };
  reset: { action: string; success: string };
  verify: {
    action: string;
    success: string;
    resendAction: string;
  ...
  account: { changeAction: string; changeSuccess: string };
}
...
export function AuthForm({
  ...
  mode: AuthFormMode;
  locale: Locale;
  messages: AuthFormMessages;
  token?: string | undefined;
  /** Defaults to the global `fetch`; pass your app's instrumented fetch. */
  fetchImpl?: typeof fetch;
  /** Where to send the browser after a successful login, per user role. */
  resolveLoginRedirect?: (role: UserRole) => string;
}) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (mode === "login" && body.user) {
        const destination = resolveLoginRedirect(body.user.role);
        router.push(
          (destination ? `/${locale}/${destination}` : `/${locale}`) as Route,
        );
        router.refresh();
        return;
      }
